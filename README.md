...
3. 在界面中填写：
   ...
   - OSS存储桶名称
   - 地域（默认：cn-shanghai）
   - **DeepSeek API Key**（可选，仅英语视频需要，用于翻译成中文）

4. 点击"开始处理"，等待完成后下载结果

**注意**：
- 处理英语视频时，如果提供了DeepSeek API Key，将自动翻译成中文字幕
- MP3音频文件会保存到视频同级目录，下次处理同一视频时可复用
- 生成的文件包括：`视频名_字幕版.mp4`、`视频名_zh.srt`（或`_en.srt`）、`视频名_audio.mp3`
